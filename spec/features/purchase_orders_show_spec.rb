...
RSpec.describe 'Purchase Order Show Page', type: :feature do
  describe 'GET /purchase_orders/:id' do
    ...
    it 'page has content' do
      visit purchase_order_path(@purchase_order.id)
      expect(page).to have_content('Purchase Order Show Page')
      expect(page).to have_content("Purchase Order ##{@purchase_order.id}")
    end

    it 'page shows purchase order details' do
      visit purchase_order_path(@purchase_order.id)
      expect(page).to have_content(@purchase_order.description)
      expect(page).to have_content(@purchase_order.quantity)
    ...
    it 'page shows purchase order items' do
      visit purchase_order_path(@purchase_order.id)
      expect(page).to have_content('Purchase Order Items')
      expect(page).to have_link(@item.name)
      expect(page).to have_content(@purchase_order_item.quantity)
    end

    it 'has back to purchase orders link' do
      visit purchase_order_path(@purchase_order.id)
      expect(page).to have_link('Back to Purchase Orders')
    end

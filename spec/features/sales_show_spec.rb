...
RSpec.describe 'Sale Show Page', type: :feature do
  describe 'GET /sales/:id' do
    ...
    it 'page has content' do
      ...
      expect(page).to have_content('Sale Show Page')
      expect(page).to have_content("Sale ##{@sale.id}")
    end

    it 'page shows sale details' do
      visit sale_path(@sale.id)
    ...
    it 'page shows sale invoices' do
      visit sale_path(@sale.id)
      expect(page).to have_content('Sale Invoices')
      expect(page).to have_link("Sale Invoice ##{@sale_invoice.id}")
      expect(page).to have_content(@user.email)
      expect(page).to have_link("Invoice ##{@invoice.id}")
    end

    it 'has back to sales link' do
      visit sale_path(@sale.id)
      expect(page).to have_link('Back to Sales')
    end
